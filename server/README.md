...
3. 관리자 화면 접속 비밀번호(토큰)도 등록한다. 아무 긴 임의 문자열이면 된다:
...
- 각 행에 **정지** 버튼(키 전체 무효화), 각 기기 옆에 **반납** 버튼(그 기기의 좌석만 반납)
...
1. `/admin/list`에서 기존 키를 **정지**한다.
2. `/admin/issue`에서 같은 구매자로 **새 키를 발급**한다 (메모에 "재발급, 사유: ..." 기록 권장).
3. 새 키를 구매자에게 전달한다.

## 5. 부정 사용(키 돌려쓰기 등) 의심 시

`/admin/list`에서 해당 키의 **정지** 버튼을 누르면 즉시 모든 활성화 요청이 `REVOKED`로 거부된다.
필요 시 새 키를 발급해 재판매할 수 있다.
