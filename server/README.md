...
3. 관리자 화면 접속 비밀번호(토큰)도 등록한다:
   ...
   > **반드시 24자 이상의 무작위 문자열**이어야 한다. 24자 미만이면 서버가 관리자 화면을
   > 열지 않고 `503`을 돌려준다 (약한 비밀번호로 발급 백오피스가 뚫리는 것을 막기 위함).
   >
   > 길이만 검사하므로 `aaaaaaaaaaaaaaaaaaaaaaaa` 같은 값도 통과한다 — **길이가 곧 안전은
   > 아니다.** 직접 짓지 말고 아래 명령으로 만든 값을 그대로 쓴다 (PowerShell):
   >
   > ```
   > [Convert]::ToBase64String((1..32|%{Get-Random -Max 256}))
   > ```
   >
   > 한글·이모지 등 영문/숫자가 아닌 문자도 동작하지만, 브라우저·터미널 인코딩 사고를
   > 피하려면 위 명령 결과(영문·숫자·기호)를 쓰는 편이 안전하다.
